...
namespace Gvr.Internal {
  public class InstantPreview : MonoBehaviour {
    ...
    void UpdateCamera(Camera camera, GameObject cameraObject, Transform cameraTransform) {
			GvrEditorEmulator editorEmulatorObj = FindObjectOfType<GvrEditorEmulator> (); 
      EyeCamera eyeCamera;

      if (!eyeCameras.TryGetValue(camera, out eyeCamera)) {
        return;
      }
			//if (editorEmulatorObj.GetComponent<GvrEditorEmulator> ().objHandController.activeAnimation) {
				//cameraTransform.localRotation=Quaternion.Euler (-22f, 0, 0);
			//} else {
				if (connected) {
					Debug.Log ("H1");
					if (GetHeadPose(out headPose, out timestamp)) {
						...
						Debug.Log ("H1=" + cameraTransform.localRotation);	
					} else {
					...
					if (GetEyeViews(out eyeViews)) {
						...
						Debug.Log ("H1.3=" + eyeCamera.leftEyeCamera.gameObject.transform.rotation);	
